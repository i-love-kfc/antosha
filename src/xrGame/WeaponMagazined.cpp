...
CWeaponMagazined::CWeaponMagazined(ESoundTypes eSoundType) : CWeapon()
{
    ...
    m_eSoundMinigun = ESoundTypes(SOUND_TYPE_WEAPON_RECHARGING | eSoundType);
#ifdef NEW_SOUNDS
...
void CWeaponMagazined::Load(LPCSTR section)
{
    ...
    if (WeaponSoundExist(section, "snd_minigun"))
        m_sounds.LoadSound(section, "snd_minigun", false, m_eSoundMinigun);
	...
	m_bcartridge_in_the_barrel = true;
	...
	m_bmisfire_reloadable = false;
	
	if (pSettings->line_exist(section, "misfire_reloadable"))
        m_bmisfire_reloadable = pSettings->r_bool(section, "misfire_reloadable");
	
	m_bminigun = false;
    m_bempty_reloaded = false;
    m_bempty_reloaded2 = false;
	
	if (pSettings->line_exist(section, "minigun"))
        m_bminigun = pSettings->r_bool(section, "minigun");
...
void CWeaponMagazined::UnloadMagazine(bool spawn_ammo)
{
    ...
    while (!m_magazine.empty())
    {
        ...
        m_magazine.pop_back();
		/*if(!m_bmisfire_reloaded)
            --m_ammoElapsed.type1;
		else
		{
            m_bmisfire_reloaded = false;
...
void CWeaponMagazined::OnAnimationEnd(u32 state)
{
    ...
    case eReload:
	{
		if (IsMisfire() && m_bmisfire_reloadable)
		{
		    bMisfire = false;
            if (m_ammoElapsed.type1 > 0)
		    {
                --m_ammoElapsed.type1;
                //m_bmisfire_reloaded = true;
		    }
		}
		else
	    {
            if (!m_bGrenadeMode)
		    {
                if (m_ammoElapsed.type1 != 0 || !m_bcartridge_in_the_barrel)
			        ReloadMagazine();
                else
		        {
                    --iMagazineSize;
...
void CWeaponMagazined::switch2_Fire()
{
    ...
    if ((OnClient() || Level().IsDemoPlay()) && !IsWorking())
	{
		if(m_bminigun)
		{
			    PlayHUDMotion("anm_minigun", true, nullptr, GetState()); 
			    //PlaySound("snd_minigun", get_LastFP());
                //switch2_Fire();
