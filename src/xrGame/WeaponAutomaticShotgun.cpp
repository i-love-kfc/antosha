...
void CWeaponAutomaticShotgun::Load(LPCSTR section)
{
    ...
    if (m_bTriStateReload)
    {
        ...
        m_sounds.LoadSound(section, "snd_close_empty", "sndCloseEmpty", false, m_eSoundClose);
    };
}
...
{
    if (m_ammoElapsed.type1 == 1 && m_bempty_reloaded && m_bcartridge_in_the_barrel && !m_bempty_reloaded2)
	{
		m_bempty_reloaded2 = true;
        m_bempty_reloaded = false;
        m_sounds.StopSound("sndAddCartridge");
